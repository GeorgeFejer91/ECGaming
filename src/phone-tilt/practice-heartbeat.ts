...
/** Consume the selected ground practice clock once per beat; never extrapolate network beats. */
export class PracticeHeartbeat {
  ...
  update(relay: RelayState | null, fresh: boolean, visible: boolean) {
    ...
    const active = Boolean(fresh && visible && relay?.source === "ground" && frame &&
      (frame.flags & FlightFlags.simulation) && (frame.flags & FlightFlags.controlReady));
