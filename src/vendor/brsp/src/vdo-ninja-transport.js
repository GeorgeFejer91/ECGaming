...
export class VdoNinjaTransport extends EventTarget {
  ...
  async openTargetChannels(peerKey) {
    ...
    peer.opening = true;
    try {
      // Attach the reliable listener before opening the second lane. A remote
      // peer can send hello as soon as it sees both lanes, before our second
      // openChannel promise resolves. Keep those early messages bounded below.
      const control = await sdk.openChannel(peerKey, VDO_BRSP_CONTROL_CHANNEL, { ordered: true });
      ...
      const state = await sdk.openChannel(peerKey, VDO_BRSP_STATE_CHANNEL, { ordered: false, maxRetransmits: 0 });
      if (!this.isCurrentSdk(sdk, generation) || this.peers.get(peerKey) !== peer) {
        try { control.close(); } catch { /* stale channel */ }
  ...
  attachChannel(peer, lane, channel) {
    ...
    channel.addEventListener("message", (event) => {
      ...
      if (!peer.opened) {
        ...
        const bytes = protocolBytes(event.data);
        if (peer.pendingControl.length >= 4 || peer.pendingControlBytes + bytes > BRSP_CONTROL_MAX_BYTES) {
          this.markPeerClosed(peer.peerKey, "Early pairing messages exceeded the limit.");
          return;
        }
        peer.pendingControl.push(event.data);
        peer.pendingControlBytes += bytes;
        return;
      }
  ...
  finishPeerOpen(peer) {
    ...
    const early = peer.pendingControl.splice(0);
    peer.pendingControlBytes = 0;
    for (const data of early) {
      if (this.peers.get(peer.peerKey) !== peer) break;
      this.dispatchEvent(detailEvent("controlmessage", { peerKey: peer.peerKey, data }));
    }
  }
