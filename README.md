...
**EC Gaming turns live ECG and chest-motion signals into browser games.** The first game, Heartbeat Flight, is a low-poly airplane experience that runs as a normal 3D web app and can enter immersive WebXR on a compatible headset.
...
- **MOTH heartbeat + dive trial** is built from the user's English-only [MOTH adaptation branch](https://github.com/GeorgeFejer91/moth-game/tree/codex/ecgaming-dive-bridge) at commit `aa9506473a856a63f19e5650656c74793865b5d1`, based on [ahmedallam222/moth-game](https://github.com/ahmedallam222/moth-game). All five chapters are immediately selectable. Fresh R-peaks jump on land. Underwater R-peak paddles are accepted only while fresh physical Polar ACC breathing input is at a stable calibrated upper crest. Keyboard, touch, and gamepad remain available as a safety fallback.
...
- The current 92-byte `ecgsignalv1` beacon contains only finite derived heart/breathing metrics, separate ECG/RR beat counters and ages, quality, and readiness/provenance flags. Receivers also accept the legacy 88-byte heart-only frame. Neither contains a raw sample array, device name, Bluetooth identifier, or participant identity.
...
public/games/          attributed third-party game adapters and launchers
games/                 Vite entry points and adapted browser-game source
